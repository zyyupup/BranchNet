...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000,scale_factor = 1.0):
        ...
        self.branch = nn.ModuleList([self._make_branch(p) for p in [16*block.expansion ,32*block.expansion]])
...
def se_resnet164(pretrained=False, **kwargs):
    model = ResNet(Bottleneck, [18, 18, 18], **kwargs)
    return model
def se_resnet272(pretrained=False, **kwargs):
    model = ResNet(Bottleneck, [30, 30, 30], **kwargs)
    return model
...
if __name__ == '__main__':
    model = se_resnet164(num_classes = 10)
